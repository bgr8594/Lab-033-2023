...
export class DestinosApiPage implements OnInit {
  ...
  getLugares() {
    this.lugarService.getLugaresApi().subscribe(
      (response: Lugar[]) => {
  ...
  submitForm() {
    if (this.ionicForm.valid) {
      ...
      if (!this.editando) {
        this.lugarService.altaLugarApi(this.lugar).subscribe(
          (response: any) => {
            if (response) {
        ...
        this.lugarService.editarLugarApi(this.lugar.id, this.lugar).subscribe(
          (resposne: any) => {
            if (resposne) {
              this.editando = false;
  ...
  eliminarLugar(id: any) {
    this.lugarService.borrarLugarApi(id).subscribe(
      (response: any) => {
        if (response) {
          this.getLugares();
          this.estado = 'Alta destino';
    ...
    return new Promise((resolve: any, reject: any): any => {
      navigator.geolocation.getCurrentPosition(
        ...
        (err: any) => {
          if (err.code === 1) {
            alert(
              'Favor de activar la geolocalización en tu navegador y recargar la pantalla.'
            );
          }
          this.latitud = null;
          this.longitud = null;
